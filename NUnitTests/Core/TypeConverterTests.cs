...
namespace UnitTests
{
    ...
    public class TypeConverterTests
    {
        ...
        public void ConvertTo()
        {
			...
			Assert.IsInstanceOf<NumberInstance>(TypeConverter.ConvertTo<ObjectInstance>(engine, 100));
            Assert.AreEqual(100.0, ((NumberInstance)TypeConverter.ConvertTo<ObjectInstance>(engine, 100)).ValueOf());
			Assert.IsInstanceOf<NumberInstance>(TypeConverter.ConvertTo<NumberInstance>(engine, 100));
        }
        ...
        public void ToObject()
        {
			...
			Assert.IsInstanceOf<BooleanInstance>(TypeConverter.ToObject(engine, false));
            Assert.AreEqual(false, TypeConverter.ToObject(engine, false).CallMemberFunction("valueOf"));

			Assert.IsInstanceOf<BooleanInstance>(TypeConverter.ToObject(engine, true));
            Assert.AreEqual(true, TypeConverter.ToObject(engine, true).CallMemberFunction("valueOf"));

			Assert.IsInstanceOf<NumberInstance>(TypeConverter.ToObject(engine, 13.9));
            Assert.AreEqual(13.9, TypeConverter.ToObject(engine, 13.9).CallMemberFunction("valueOf"));

			Assert.IsInstanceOf<StringInstance>(TypeConverter.ToObject(engine, "test"));
